      ...
      <section className="px-6 py-20">
        <div className="max-w-5xl mx-auto grid gap-8">
          {posts.map((post) => (
            <article
              key={post.slug}
              className="bg-white border border-stone-200 rounded-sm p-8 shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <Link
                href={`/blog/${post.slug}`}
                className="inline-flex items-center gap-2 text-amber-700 hover:text-amber-600 font-bold text-sm uppercase tracking-widest"
              >
